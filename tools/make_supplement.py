...
def main() -> None:
    parts = [
        ...
        # sn-jnl sets up two-sided pages: a 6 mm binding offset on the inner side and a
        # 2:3 inner:outer margin split, so the text block jumps sideways on every other
        # page. The PDF is read on screen; centre it and keep it in one place.
        "\\geometry{bindingoffset=0mm, hmarginratio=1:1}",
        "\\usepackage{amsmath}",
        "\\usepackage{graphicx}",
        "\\usepackage{framed}",
        "\\usepackage{multirow}",
